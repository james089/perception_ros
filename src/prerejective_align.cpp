...
{
  std::string fname1 = std::string (DATA_PATH + "/" + argv[1]);
  std::string fname2 = std::string (DATA_PATH + "/" + argv[2]);
  // Point clouds
  ...
  if (pcl::io::loadPCDFile<PointNT> (fname1, *object) < 0 ||
      pcl::io::loadPCDFile<PointNT> (fname2, *scene) < 0)
  {
