...
void loadData (int argc, char** argv, std::vector<PCD, Eigen::aligned_allocator<PCD> > &models, const std::string path)
{
  std::string extension (".pcd");
  // Suppose the first argument is the actual test model
  for (int i = 1; i < argc; i++)
  {
    std::string fname = std::string (path + "/" + argv[i]);
    //cout << fname;
...
void RunRegister(int argc, char **argv, const std::string path)
{
    // Load data
    std::vector<PCD, Eigen::aligned_allocator<PCD> > data;
    loadData(argc, argv, data, path);
    ...
    for (std::size_t i = 1; i < data.size (); ++i)
    {
        ...
        std::stringstream ss;
        ss << i << ".pcd";
        pcl::io::savePCDFile (path + "/" + ss.str(), *result, true);
...
{
    // ROS params
    ros::init(argc, argv, "register_cloud_whole");
    ...
    RunRegister(argc, argv, mConfig.data_path);
